...
# This script builds the project and generates Lider (lider-script.jar) & Lider Console (lider-console-script.jar) distribution files as well as Ahenk package (script.deb)
#
# Generated files can be found under /tmp/lider-ahenk-script-plugin/
set -e
...
cd "$PRJ_ROOT_PATH"/ahenk-script
dpkg-buildpackage -b -uc
echo "Generated Ahenk package"

EXPORT_PATH=/tmp/lider-ahenk-script-plugin
...
cp -rf "$PRJ_ROOT_PATH"/lider-script/target/lider-*.jar "$EXPORT_PATH"
cp -rf "$PRJ_ROOT_PATH"/lider-console-script/target/lider-console-*.jar "$EXPORT_PATH"
